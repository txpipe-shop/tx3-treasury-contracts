...
export async function setupBlaze(txOuts: Core.TransactionOutput[] = []) {
    ...
    scriptRef.setScriptRef(treasuryScript)
    ...
    const protocolParameters = {
        ...
        costModels: (/* @__PURE__ */ new Map()).set(
            ...
            [
                ...
                100, 16e3, 100, 100, 100, 16e3, 100, 94375, 32, 132994, 32, 61462,
                4, 72010, 178, 0, 1, 22151, 32, 91189, 769, 4, 2, 85848, 228465, 122,
                0, 1, 1, 1e3, 42921, 4, 2, 24548, 29498, 38, 1, 898148, 27279, 1,
                51775, 558, 1, 39184, 1e3, 60594, 1, 141895, 32, 83150, 32, 15299, 32,
                76049, 1, 13169, 4, 22100, 10, 28999, 74, 1, 28999, 74, 1, 43285, 552,
                1, 44749, 541, 1, 33852, 32, 68246, 32, 72362, 32, 7243, 32, 7391, 32,
                11546, 32, 85848, 228465, 122, 0, 1, 1, 90434, 519, 0, 1, 74433, 32, 85848,
                228465, 122, 0, 1, 1, 85848, 228465, 122, 0, 1, 1, 270652, 22588, 4,
            ...
            [
                ...
                100, 16e3, 100, 100, 100, 16e3, 100, 94375, 32, 132994, 32, 61462,
                4, 72010, 178, 0, 1, 22151, 32, 91189, 769, 4, 2, 85848, 228465, 122,
                0, 1, 1, 1e3, 42921, 4, 2, 24548, 29498, 38, 1, 898148, 27279, 1, 51775,
                ...
                85848, 228465, 122, 0, 1, 1, 85848, 228465, 122, 0, 1, 1, 955506, 213312,
...
export function makeExpectTxValid(blaze: Blaze<Provider, Wallet>, emulator: Emulator): (tx: TxBuilder) => Promise<void> {
    return async (tx: TxBuilder) => {
        ...
        await emulator.awaitTransactionConfirmation(txId)
        expect(txId).toBeDefined()
    }
}
export function makeExpectTxInvalid(blaze: Blaze<Provider, Wallet>, emulator: Emulator): (tx: TxBuilder) => Promise<void> {
    return async (tx: TxBuilder) => {
        expect(async () => {
            const completedTx = await tx.complete()
            const signedTx = await blaze.signTransaction(completedTx)
